...
void loop() {
  ...
  if (mudandoVEL) {
    ...
    if (posicao_atual_M != posicao_antiga_M) {
      velocidade = alterandoParametro(posicao_atual_M, posicao_antiga_M, velocidade, intervalo_M, 10, 2000); 
    ...
    if (posicao_atual_E != posicao_antiga_E) {
      temperatura = alterandoParametro(posicao_atual_E, posicao_antiga_E, temperatura, intervalo_E, 10, 2000);
...
int alterandoParametro(long posicao_atual, long& posicao_anterior, int parametro, int& intervalo, int passo, int limite) {
  if (posicao_atual != posicao_anterior) {
    if (posicao_atual > posicao_anterior) {
      parametro += passo;
      if (parametro > limite) parametro = limite;

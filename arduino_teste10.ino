...
int alterandoTemp() {
  if (posicao_atual_E != posicao_anterior_E) {
    ...
    if (posicao_atual_E > posicao_anterior_E) {
      ...
      if (temperatura > 2000) velocidade = 2000; // Limita a velocidade máxima em 1000 passos por segundo
      intervalo_E = 2000 / temperatura; // Atualiza o intervalo_E entre os passos
    }
    // Se a posição atual for menor que a anterior, significa que o encoder rotativo girou no sentido anti-horário
    else if (posicao_atual_E < posicao_anterior_E) {
      temperatura -= 10; // Diminui a velocidade do motor em 10 passos por segundo
      if (temperatura < 0) temperatura = 0; // Limita a velocidade mínima em 0 passos por segundo
      intervalo_E = 2000 / temperatura; // Atualiza o intervalo_E entre os passos
    }
    posicao_anterior_E = posicao_atual_E; // Atualiza a posição anterior do encoder rotativo
  }
  return velocidade;
}
